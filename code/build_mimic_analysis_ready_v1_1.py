...
def main() -> None:
    ...
    con.execute(
        ...
        FROM grids g JOIN history h ON h.stay_id=g.stay_id AND h.grid_hour=g.grid_hour AND h.interval_index=0
    ...
    con.execute(
        ...
        JOIN history h ON h.stay_id=g.stay_id AND h.grid_hour=g.grid_hour AND h.interval_index=t.interval_index-1
        """
    )

    grid_path = restricted_derived / "mimic_analysis_grid_v1_1.csv.gz"
    interval_path = restricted_derived / "mimic_analysis_intervals_v1_1.csv.gz"
    con.execute(
    ...
    metadata = {
        ...
        "protocol": "PROTOCOL_v1.0.md + PROTOCOL_AMENDMENT_v1.1_PREOUTCOME.md",
