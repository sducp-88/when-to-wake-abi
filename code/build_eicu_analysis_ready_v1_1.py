...
def main() -> None:
    ...
    con.execute(
        f"""
        CREATE TEMP TABLE apache_iva AS
        SELECT patientunitstayid,apache_iva_aps,apache_iva_score
        FROM (
          SELECT TRY_CAST(patientunitstayid AS BIGINT) AS patientunitstayid,
                 TRY_CAST(acutephysiologyscore AS DOUBLE) AS apache_iva_aps,
                 TRY_CAST(apachescore AS DOUBLE) AS apache_iva_score,
                 ROW_NUMBER() OVER (
                   PARTITION BY TRY_CAST(patientunitstayid AS BIGINT)
                   ORDER BY TRY_CAST(apachepatientresultsid AS BIGINT)
                 ) AS rn
          FROM read_csv(?,{csv_opts})
          WHERE LOWER(COALESCE(apacheversion,''))='iva'
        ) ranked
        WHERE rn=1
        """,
        [str(eicu / "apachePatientResult.csv.gz")],
    )
    ...
    con.execute(
        ...
        SELECT f.*,a.tbi,a.sah,a.ich,a.ais,a.primary_hypoxic_arrest,v.first_vent_start,
               ap.apache_iva_aps,ap.apache_iva_score,
        ...
        LEFT JOIN apache_iva ap USING(patientunitstayid)
    ...
    con.execute(
        ...
        WITH hemodynamics AS (
          ...
          SELECT p.patientunitstayid,p.grid_hour,p.interval_index,
                 ...
                 ARG_MAX(n.value_num,n.observationoffset)
                   FILTER(WHERE n.label='glasgow coma score gcs total'
                                AND n.value_num BETWEEN 3 AND 15) AS gcs_value,
                 COUNT(n.observationoffset)
                   FILTER(WHERE (n.label LIKE '%rass%' AND n.value_num BETWEEN -5 AND 4)
                                OR (n.label='glasgow coma score gcs total'
                                    AND n.value_num BETWEEN 3 AND 15)) AS neuro_measure_count
    ...
    grid_path = restricted_derived / "eicu_analysis_grid_transport_v1_1.csv.gz"
    interval_path = restricted_derived / "eicu_analysis_intervals_transport_v1_1.csv.gz"
    con.execute(
    ...
    metadata = {
        ...
        "gcs_rule": "latest pre-decision Glasgow coma score GCS Total with valid range 3-15; invalid values excluded",
        "apache_iva_role": "baseline severity diagnostic; primary transport model uses time-aligned pre-decision components",
    }
    (restricted_derived / "eicu_analysis_ready_metadata_v1_1.json").write_text(
