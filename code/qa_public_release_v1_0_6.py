...
def main() -> None:
    ...
    for path in sorted(release_files(root)):
        ...
        is_release_qa_script = path.name.startswith("qa_public_release_v1_0_") and path.suffix.lower() == ".py"
        if not is_release_qa_script and path.name not in {"PUBLIC_RELEASE_QA.json", "MANIFEST_SHA256.csv"} and path.suffix.lower() in {".md", ".txt", ".py", ".json", ".csv", ".cff"}:
    ...
    required = [
        ...
        "publication_assets/figures/Graphical_Abstract.png",
        "publication_assets/figures/Graphical_Abstract.svg",
    ]
    missing_required_files = [item for item in required if not (root / item).is_file()]
    graphical_svg = (root / "publication_assets/figures/Graphical_Abstract.svg")
    graphical_text = graphical_svg.read_text(encoding="utf-8", errors="ignore") if graphical_svg.is_file() else ""
    graphical_abstract_consistent = (
        "+4.0 pp" in graphical_text
        and "95% CI -2.5 to 11.1" in graphical_text
        and "+3.2 pp" not in graphical_text
        and "95% CI -2.9 to 10.4" not in graphical_text
    )
    ...
    result = {
        ...
        "graphical_abstract_consistent_with_eicu_v1_2": graphical_abstract_consistent,
        "license_status": "APPROVED_MIT",
        "patient_level_data_included": False,
        "failed_gate_apache_effect_included": False,
    }
    if forbidden_content_hits or restricted_filename_hits or syntax_errors or missing_required_files or not graphical_abstract_consistent:
        result["status"] = "FAIL"
