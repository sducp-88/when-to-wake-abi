...
GRID_PATH = PROJECT_ROOT / "00_restricted_data" / "derived" / "eicu_transport_v1_1" / "eicu_analysis_grid_transport_v1_1.csv.gz"
INTERVAL_PATH = PROJECT_ROOT / "00_restricted_data" / "derived" / "eicu_transport_v1_1" / "eicu_analysis_intervals_transport_v1_1.csv.gz"
RESTRICTED_DERIVED = PROJECT_ROOT / "00_restricted_data" / "derived" / "eicu_transport_v1_1"
...
def harmonize(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    gcs_total = numeric_series("gcs_value")
    frame["gcs_motor"] = gcs_total.where(gcs_total.between(3, 15))
    frame["gcs_eye"] = np.nan
    frame["gcs_verbal"] = np.nan
    gcs_total_1 = numeric_series("gcs_value_1")
    frame["gcs_motor_1"] = gcs_total_1.where(gcs_total_1.between(3, 15))
    frame["gcs_eye_1"] = np.nan
    frame["gcs_verbal_1"] = np.nan
    frame["apache_iva_score_1"] = numeric_series("apache_iva_score")
...
def rake_transport_weights(final: pd.DataFrame, target: pd.DataFrame) -> pd.DataFrame:
    ...
    margins = [
        "baseline_drug_count",
        "icp_calibration_bin",
        "fio2_calibration_bin",
        "abi_subtype",
        "apache_iva_calibration_bin",
        "pao2_fio2_calibration_bin",
        "lactate_calibration_bin",
        "map_min_calibration_bin",
        "gender",
        "opioid_observed",
        "midazolam_active",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--label", default="transport_v1_2_late24h_apache")
    ...
    module.NUMERIC_DENOMINATOR = list(dict.fromkeys(module.NUMERIC_DENOMINATOR + ["apache_iva_score_1"]))
    module.BALANCE_NUMERIC = list(dict.fromkeys(
        module.BALANCE_NUMERIC + ["apache_iva_aps", "apache_iva_score"]
    ))
    ...
    grid = grid[
        pd.to_numeric(grid["grid_offset"], errors="coerce").ge(1440)
        & pd.to_numeric(grid["apache_iva_score"], errors="coerce").notna()
    ].copy()
    intervals = intervals[
        pd.to_numeric(intervals["grid_offset"], errors="coerce").ge(1440)
        & pd.to_numeric(intervals["apache_iva_score"], errors="coerce").notna()
    ].copy()
    if grid.empty or intervals.empty:
        raise RuntimeError("No time-valid APACHE IVa decision points remained")
    for frame in (grid, intervals):
        frame["apache_iva_calibration_bin"] = pd.cut(
            pd.to_numeric(frame["apache_iva_score"], errors="coerce"),
            bins=[-np.inf, 50, 70, 90, np.inf],
            labels=["le50", "50to70", "70to90", "gt90"],
        ).astype("string").fillna("missing")
        frame["pao2_fio2_calibration_bin"] = pd.cut(
            pd.to_numeric(frame["pao2_fio2"], errors="coerce"),
            bins=[-np.inf, 100, 150, 200, 250, 300, 400, np.inf],
            labels=["le100", "100to150", "150to200", "200to250", "250to300", "300to400", "gt400"],
        ).astype("string").fillna("missing")
        frame["lactate_calibration_bin"] = pd.cut(
            pd.to_numeric(frame["lactate"], errors="coerce"),
            bins=[-np.inf, 2, 4, np.inf],
            labels=["le2", "2to4", "gt4"],
        ).astype("string").fillna("missing")
        frame["map_min_calibration_bin"] = pd.cut(
            pd.to_numeric(frame["map_min_2h"], errors="coerce"),
            bins=[-np.inf, 65, 70, 80, 90, np.inf],
            labels=["le65", "65to70", "70to80", "80to90", "gt90"],
        ).astype("string").fillna("missing")
    ...
    max_smd = float(balance["smd_weighted"].abs().max())
    min_ess_ratio = float(weights["ess_ratio"].min())
    max_weight = float(weights["truncated_weight_max"].max())
    gates_passed = max_smd < 0.10 and min_ess_ratio >= 0.50 and max_weight < 20

    metadata = {
        "study_id": "WHEN-TO-WAKE-ABI",
        "database": "eICU-CRD-2.0",
        "analysis_role": "post hoc time-valid APACHE IVa severity sensitivity",
        "analysis_version": "1.2",
        "run_at_utc": datetime.now(timezone.utc).isoformat(),
        "bootstrap_requested": args.bootstrap if gates_passed else 0,
        ...
        "post_weight_calibration": "iterative raking to outcome-blind baseline drug-count, ICP-bin, FiO2-bin, ABI-subtype, APACHE IVa-bin, PaO2/FiO2-bin, lactate-bin, MAP-min-bin, gender, opioid-observed, and midazolam-active margins",
        "gcs_rule": "eICU GCS Total restricted to valid values 3-15 before modeling",
        "apache_iva_role": "included in the denominator model only for complete-case decisions at or after ICU minute 1440, after completion of the APACHE first-day window",
        "decision_time_restriction": "grid_offset >= 1440 minutes from ICU admission",
        "post_hoc_sensitivity": True,
        "patient_level_output": str(restricted_path),
        "diagnostic_gates": {
            "maximum_absolute_weighted_smd": max_smd,
            "minimum_ess_ratio": min_ess_ratio,
            "maximum_weight": max_weight,
            "passed": gates_passed,
        },
        "effect_estimate_released": False,
    }
    metadata_path = RESULT_DIR / f"{prefix}_run_metadata.json"
    print(weights.to_string(index=False))
    print(f"MAX_ABS_WEIGHTED_SMD={max_smd:.4f}")
    print(f"WEIGHTS={weight_path}")
    print(f"BALANCE={balance_path}")
    print(f"CALIBRATION={calibration_path}")
    print(f"RESTRICTED_RETAINED_CLONES={restricted_path}")
    if not gates_passed:
        metadata["status"] = "FAIL_NONINFERENTIAL"
        metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")
        print("Diagnostic gates failed; no effect estimate or confidence interval was computed or written.")
        return
    ...
    metadata["bootstrap_converged"] = int(bootstrap["converged"].sum()) if len(bootstrap) else 0
    metadata["effect_estimate_released"] = True
    metadata["safe_effect_output"] = str(effect_path)
    metadata["status"] = "PASS_INFERENTIAL"
    metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")
