...
def main() -> None:
    ...
    check("grid_rows_positive", int(len(grid) > 0), 1)
    ...
    violations = {
        ...
        "gcs_total_outside_valid_3_to_15": (
            pd.to_numeric(grid["gcs_value"], errors="coerce").notna()
            & ~pd.to_numeric(grid["gcs_value"], errors="coerce").between(3, 15)
        ),
    }
    ...
    qa_path = RESULT_DIR / "eicu_analysis_ready_qa_v1_1.csv"
    ...
    summary = {
        ...
        "gcs_rule": "GCS Total restricted to 3-15",
        "apache_iva_score_coverage_percent": round(
            float(100 * pd.to_numeric(grid["apache_iva_score"], errors="coerce").notna().mean()), 4
        ),
    }
    metadata_path = RESULT_DIR / "eicu_analysis_ready_qa_v1_1.json"
