...
GRID_PATH = PROJECT_ROOT / "00_restricted_data" / "derived" / "mimic_v1_5_bias_targeted" / "mimic_analysis_grid_v1_5.csv.gz"
INTERVAL_PATH = PROJECT_ROOT / "00_restricted_data" / "derived" / "mimic_v1_5_bias_targeted" / "mimic_analysis_intervals_v1_5.csv.gz"
RESTRICTED_DERIVED = PROJECT_ROOT / "00_restricted_data" / "derived" / "mimic_v1_5_bias_targeted"
RESULT_DIR = Path(
    os.environ.get(
        "WTW_RESULT_DIR",
        PROJECT_ROOT / "05_results" / "formal_analysis" / "mimic",
    )
).resolve()
...
NUMERIC_DENOMINATOR = [
    ...
    "propofol_baseline_log_rate",
    "midazolam_baseline_log_rate",
    "dexmed_baseline_log_rate",
    "delta_map_median_6h_1",
    "delta_map_min_6h_1",
    "delta_heart_rate_6h_1",
    "delta_fio2_6h_1",
    "delta_peep_6h_1",
    "delta_rass_6h_1",
    "delta_gcs_motor_6h_1",
    "delta_gcs_eye_6h_1",
    "delta_gcs_verbal_6h_1",
    "delta_icp_latest_6h_1",
    "delta_neuro_measure_count_6h_1",
]
...
CATEGORICAL_DENOMINATOR = [
    ...
    "ketamine_observed_1",
    "any_sbt_prior6h_1",
    "propofol_baseline_unit",
    "midazolam_baseline_unit",
    "dexmed_baseline_unit",
    "vasopressor_observed_prev6h_1",
    "opioid_observed_prev6h_1",
    "ketamine_observed_prev6h_1",
]
...
BALANCE_CATEGORICAL = [
    ...
    "midazolam_baseline_unit",
    "dexmed_baseline_unit",
    "vasopressor_observed_prev6h",
    "opioid_observed_prev6h",
    "ketamine_observed_prev6h",
]
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for frame in (grid, intervals):
        for drug in ("propofol", "midazolam", "dexmed"):
            rate = pd.to_numeric(frame[f"{drug}_baseline_rate"], errors="coerce")
            frame[f"{drug}_baseline_log_rate"] = np.log1p(rate.clip(lower=0)).fillna(0.0)
            frame[f"{drug}_baseline_unit"] = frame[f"{drug}_baseline_unit"].fillna("inactive").astype(str)
...
def add_eligible_decision_count(intervals: pd.DataFrame) -> pd.DataFrame:
    frame = intervals.copy()
    stay_key = "analysis_stay_id" if "analysis_stay_id" in frame.columns else "stay_id"
    first_intervals = frame.loc[frame["interval_index"].eq(1), [stay_key, "grid_hour"]].drop_duplicates()
    counts = first_intervals.groupby(stay_key, observed=True).size().rename("eligible_decision_count")
    frame["eligible_decision_count"] = frame[stay_key].map(counts).astype(float)
    if frame["eligible_decision_count"].isna().any() or frame["eligible_decision_count"].le(0).any():
        raise RuntimeError("Invalid eligible-decision count for stay-balanced sensitivity")
    return frame


def apply_stay_balance(final: pd.DataFrame) -> pd.DataFrame:
    balanced = final.copy()
    balanced["weight"] = balanced["weight"].astype(float) / balanced["eligible_decision_count"].astype(float)
    balanced["stay_balanced"] = 1
    return balanced
...
def bootstrap_once(interval_sample: pd.DataFrame, seed: int, stay_balanced: bool = False) -> dict[str, float]:
    try:
        interval_sample = add_eligible_decision_count(interval_sample)
        final, _ = fit_weights(interval_sample, collect_diagnostics=False)
        estimate = effect_from_final(final)
        stay_estimate = effect_from_final(apply_stay_balance(final))
        estimate.update({f"stay_balanced_{key}": value for key, value in stay_estimate.items()})
        estimate["seed"] = seed
        ...
        return {
            ...
            "risk_ratio": np.nan,
            "stay_balanced_risk_continue": np.nan,
            "stay_balanced_risk_early": np.nan,
            "stay_balanced_risk_difference": np.nan,
            "stay_balanced_risk_ratio": np.nan,
            "seed": seed,
            "converged": 0,
        }
...
def main() -> None:
    ...
    parser.add_argument("--label", default="bias_targeted_v1_5")
    args = parser.parse_args()

    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    RESTRICTED_DERIVED.mkdir(parents=True, exist_ok=True)
    grid, intervals = load_data()
    intervals = add_eligible_decision_count(intervals)
    final, calibration_frame = fit_weights(intervals, collect_diagnostics=True)
    estimate = effect_from_final(final)
    stay_final = apply_stay_balance(final)
    stay_estimate = effect_from_final(stay_final)
    weights = pd.concat(
        [
            weight_diagnostics(final).assign(analysis="trajectory_dose_augmented"),
            weight_diagnostics(stay_final).assign(analysis="trajectory_dose_augmented_stay_balanced"),
        ],
        ignore_index=True,
    )
    balance = pd.concat(
        [
            balance_diagnostics(final).assign(analysis="trajectory_dose_augmented"),
            balance_diagnostics(stay_final).assign(analysis="trajectory_dose_augmented_stay_balanced"),
        ],
        ignore_index=True,
    )
    ...
    output_rows = []
    for analysis, values, prefix_key in (
        ("trajectory_dose_augmented", estimate, ""),
        ("trajectory_dose_augmented_stay_balanced", stay_estimate, "stay_balanced_"),
    ):
        for key in ("risk_continue", "risk_early", "risk_difference", "risk_ratio"):
            lower, upper = interval_confidence(bootstrap, f"{prefix_key}{key}")
            output_rows.append(
                {
                    "analysis": analysis,
                    "estimand": key,
    ...
    trajectory_columns = [
        ...
        "delta_icp_latest_6h",
        "delta_neuro_measure_count_6h",
        "propofol_baseline_rate",
        "midazolam_baseline_rate",
        "dexmed_baseline_rate",
    ]
    missingness = pd.DataFrame(
        [
            {
                "variable": column,
                "n_eligible_decisions": int(len(grid)),
                "n_observed": int(grid[column].notna().sum()),
                "percent_missing": float(100 * grid[column].isna().mean()),
            }
    ...
    missingness_path = RESULT_DIR / f"{prefix}_trajectory_missingness.csv"
    restricted_weights_path = RESTRICTED_DERIVED / f"RESTRICTED_{prefix}_retained_clone_weights.csv.gz"
    restricted_stay_weights_path = RESTRICTED_DERIVED / f"RESTRICTED_{prefix}_stay_balanced_retained_clone_weights.csv.gz"
    ...
    stay_final.to_csv(restricted_stay_weights_path, index=False, encoding="utf-8-sig", compression="gzip")
    ...
    metadata = {
        ...
        "posthoc_status": True,
        "bias_target": "predecision improvement and baseline sedative dose",
        "trajectory_window_hours": 6,
        "stay_balanced_sensitivity": "inverse number of eligible decisions contributed by each ICU stay",
    ...
    for analysis, frame in balance.groupby("analysis", observed=True):
        print(f"MAX_ABS_WEIGHTED_SMD_{analysis}={frame['smd_weighted'].abs().max():.4f}")
